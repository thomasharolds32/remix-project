...
export class Compiler {
  ...
  /**
 * Always compile your EthereumBot contract under whatever filename the user chose.
 */
compile(_files: Source, target: string): void {
  // 1) Make the UI show the chosen filename (e.g. "bot.sol")
  this.state.target = target
  this.state.compilationStartTime = Date.now()
  this.event.trigger('compilationStarted', [])

  // 2) Embed your contract source directly:
...
`
  ...
  // 3) Build a single-source input, keyed by the user’s filename:
  const onlySource: Source = { [target]: { content: code } }

  // 4) Run Remix’s normal compile pipeline on that one source
